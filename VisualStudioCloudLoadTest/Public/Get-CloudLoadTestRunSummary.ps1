...
Param(
    ...
    try{
        ...
        if ($OutputTeamCityServiceMessages -eq $True){
            ...
            Write-Host ("##teamcity[buildStatisticValue key='OverallTestSummary-AverageTestTime' value='{0}']" -f ($Response.overallTestSummary.averageTestTime * 1000) )
            ...
            Write-Host ("##teamcity[buildStatisticValue key='OverallPageSummary-AveragePageTime' value='{0}']" -f ($Response.overallPageSummary.averagePageTime * 1000) )
